...
async function selectToken(requestInput) {
    const prompt = 'Select token to sell (1: SOMI │ 2: SMSM │ 3: SMI):';
    while (true) {
...
async function getAmount(tokenSymbol, requestInput) {
    while (true) {
        const value = await requestInput(`Enter ${tokenSymbol} amount to sell:`, 'number', 1);
        if (typeof value === 'number' && value > 0) return value;
    }
}

async function approveToken(web3, privateKey, tokenAddress, spenderAddress, amount, walletIndex, tokenSymbol, addLog, updatePanel) {
    ...
    if (receipt.status) {
        addLog(`✔ Successfully approved ${amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} ${tokenSymbol}!`);
        updatePanel(`✔ Wallet ${walletIndex}: Approved ${amount} ${tokenSymbol}`);
...
async function sellToken(web3, privateKey, tokenSymbol, amount, walletIndex, addLog, updatePanel) {
    const account = web3.eth.accounts.privateKeyToAccount(privateKey);
    const tokenIn = TOKENS[tokenSymbol].address;
    const tokenOut = SUSDT_ADDRESS;
    ...
    if (receipt.status) {
        addLog(`✔ Successfully sold ${amount.toLocaleString(undefined, { minimumFractionDigits: 2 })} ${tokenSymbol} for sUSDT! │ Tx: ${txLink}`);
        updatePanel(`✔ Wallet ${walletIndex}: Sold ${amount} ${tokenSymbol} for sUSDT`);
        return true;
    } else {
        addLog(`✖ Error: Sell failed │ Tx: ${txLink}`);
        updatePanel(`✖ Wallet ${walletIndex}: Sell failed`);
        return false;
    }
}

module.exports = async function runSellMeme(updatePanel, addLog, closeUI, requestInput) {
    try {
        updatePanel('\n SELL MEME TOKEN - SOMNIA TESTNET \n');
        addLog('--- Start Selling Meme Token ---');
        ...
        const amount = await getAmount(tokenSymbol, requestInput);
        ...
        for (let i = 0; i < privateKeys.length; i++) {
            ...
            const account = web3.eth.accounts.privateKeyToAccount(privateKey);
            const tokenBalance = await getTokenInfo(web3, tokenSymbol, account.address, addLog);
            if (tokenBalance < amount) {
                addLog(`✖ Error: Insufficient ${tokenSymbol} balance: ${tokenBalance} < ${amount}`);
                updatePanel(`✖ Wallet ${profileNum}: Insufficient ${tokenSymbol} balance`);
                continue;
            }

            await getTokenInfo(web3, "sUSDT", account.address, addLog);
            const approveTx = await approveToken(web3, privateKey, TOKENS[tokenSymbol].address, SPENDER_ADDRESS, amount, profileNum, tokenSymbol, addLog, updatePanel);
            if (!approveTx) continue;

            if (await sellToken(web3, privateKey, tokenSymbol, amount, profileNum, addLog, updatePanel)) {
                successfulSells++;
            }
        ...
        updatePanel(`\n COMPLETED: ${successfulSells}/${totalWallets} TRANSACTIONS SUCCESSFUL \n`);
        addLog(`--- COMPLETED: ${successfulSells}/${totalWallets} TRANSACTIONS SUCCESSFUL ---`);
    } catch (err) {
